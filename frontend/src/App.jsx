    ...
    socket.on('contact_updated', (updatedContact) => {
      setContacts(prev => {
        if (updatedContact.status === 'none') {
            return prev.filter(c => c.id !== updatedContact.id);
        }
              ...
              <div style={{ flex: 1, padding: '20px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'center', justifyContent: currentRelationship.status === 'accepted' ? 'flex-start' : 'center' }}>
                {currentRelationship.status === 'none' && (
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <button onClick={() => sendInvite(selectedUser.id)} style={{ padding: '10px 20px', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>Send Invitation</button>
                  </div>
                )}
                {currentRelationship.status === 'pending_sent' && <div style={{ textAlign: 'center', color: '#888' }}><h2>Invitation Sent</h2></div>}
